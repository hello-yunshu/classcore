...
export function mountTeacherLibrary(root: HTMLElement): void {
    ...
    headerActions.append(button('新建课件', () => createBlank(), 'primary-action'), button('导入 PPTX', () => importInput.click(), 'secondary-action'), importInput);
    ...
    function render(): void {
        ...
        for (const project of visible) {
            ...
            actions.append(button('编辑', () => { window.location.href = authoringUrl(project.presentationId); }, 'card-primary'), button('重命名', () => rename(project), 'card-secondary'), button('创建副本', () => duplicate(project), 'card-secondary'), button('版本历史', () => showHistory(project), 'card-secondary'), button('导出 PPTX', async () => {
...
function mountTeacherControl(root: HTMLElement, sessionId: string): void {
    ...
    function connect(): void {
        ...
        socket.addEventListener('open', () => socket?.send(JSON.stringify({ type: 'hello', role: 'teacher', clientId: `teacher-${crypto.randomUUID()}`, sessionId })));
        ...
        socket.addEventListener('close', () => { status.textContent = '控制连接已断开'; });
    }
