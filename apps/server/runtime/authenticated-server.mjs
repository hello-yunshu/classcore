...
function normalizeStudentParticipantHint(value) {
  const hint = typeof value === "string" ? value.trim() : "";
  if (!hint || !/^\d+$/.test(hint)) return hint;
  const numericMatch = credentials.find((item) => item.role === "student" && (item.participantId.endsWith(`:${hint}`) || item.participantId.endsWith(`:S${hint}`)));
  return numericMatch?.participantId ?? hint;
}
function resolveStudentJoinRequest(body) {
  const participantHint = normalizeStudentParticipantHint(body.participantHint);
  const request = { ...body, participantHint };
  const credential = body.credential;
  if (!participantHint || !credential || typeof credential.value !== "string") return request;
  const bootstrap = credentials.find((item) => item.role === "student" && item.sessionLocator === body.sessionLocator && item.credentialType === credential.type && item.credentialValue === credential.value);
  const target = credentials.find((item) => item.role === "student" && item.sessionLocator === body.sessionLocator && item.participantId === participantHint);
  if (!bootstrap || !target) return request;
  return { ...request, credential: { ...credential, value: target.credentialValue } };
}
...
function activityControlOptions() {
  return activities.map((activity) => {
    const layout = activity.applets.map((item) => configs[item.configRef]?.payload?.studentSurfaceLayout).find(Boolean);
    if (layout !== "two-fragment" && layout !== "four-fragment") return null;
    return { activityId: activity.activityId, label: layout === "two-fragment" ? "第一阶段" : "第二阶段" };
  }).filter(Boolean);
}
...
const eventRegistry = {
  getInstancePolicy(appletInstanceId) {
    const instance = activities.flatMap((activity) => activity.applets).find((item) => item.appletInstanceId === appletInstanceId);
...
const appletAccess = {
  getAccessMode(sessionId, _activityId, participantId, appletInstanceId) {
    const activity = runtime.currentActivity(sessionId)?.activity ?? currentActivity;
    return activity.applets.some((item) => item.appletInstanceId === appletInstanceId) && participantId.startsWith("student:") ? "interactive" : null;
  },
};
...
async function startCourse(courseId, baseUrl = publicBaseUrl) {
  ...
  const displayUrl = new URL("/display/index.html", baseUrl);
  displayUrl.searchParams.set("mode", "classroom");
  displayUrl.searchParams.set("sessionId", nextSession.sessionId);
  displayUrl.searchParams.set("locator", nextLocator);
  displayUrl.searchParams.set("code", displayCredentialValue);
  return {
    ...
    joinUrls: {
      teacher: `${baseUrl}/teacher/index.html?mode=classroom&locator=${encodeURIComponent(nextLocator)}&code=${encodeURIComponent(credentialValueFor("teacher"))}`,
      display: displayUrl.toString(),
      observer: `${baseUrl}/observer/index.html?session=${encodeURIComponent(nextLocator)}&code=${encodeURIComponent(credentialValueFor("observer"))}`,
      student: `${baseUrl}/student/index.html?mode=classroom&session=${encodeURIComponent(nextLocator)}&code=${encodeURIComponent(credentialValueFor("student"))}`,
    },
  };
}
...
function broadcastCurrentActivity(sessionId, activity) {
  broadcast(sessionId, { type: "activity.current", activity }, (item) => item.role === "student" || item.role === "teacher");
}
...
function publicSubjectId(sessionId, participantId) {
  const subjects = publicSubjectsBySession.get(sessionId) ?? new Map();
  publicSubjectsBySession.set(sessionId, subjects);
  if (!subjects.has(participantId)) subjects.set(participantId, `anon:${String(subjects.size + 1).padStart(2, "0")}`);
  return subjects.get(participantId);
}
function liveFrameObjects(item) {
  const frame = item?.frame && typeof item.frame === "object" ? item.frame : item;
  const payload = frame?.payload && typeof frame.payload === "object" ? frame.payload : {};
  const state = payload.state && typeof payload.state === "object" ? payload.state : null;
  const objects = state?.objects && typeof state.objects === "object" ? state.objects : payload.objects;
  return objects && typeof objects === "object" ? objects : {};
}
function publicLiveFrame(sessionId, item) {
  const frame = item?.frame && typeof item.frame === "object" ? item.frame : {};
  const objects = Object.fromEntries(Object.entries(liveFrameObjects(item)).slice(0, 5).map(([id, value]) => [id, value]));
  return { subjectId: publicSubjectId(sessionId, item.participantId), seq: Number.isInteger(item.seq) ? item.seq : 0, payload: { objects } };
}
function selectedLiveWidget(sessionId, participantIds) {
  ...
  const liveFrames = selected.map((item) => publicLiveFrame(sessionId, item));
  return {
...
function publicBaseUrlForRequest(req) {
  if (process.env.CLASSROOM_PUBLIC_BASE_URL) return publicBaseUrl;
  const hostHeader = typeof req?.headers?.host === "string" ? req.headers.host.trim() : "";
  if (!/^(?:localhost|[a-z0-9.-]+|\[[0-9a-f:]+\])(?::\d{1,5})?$/i.test(hostHeader)) return publicBaseUrl;
  const forwardedProto = typeof req?.headers?.["x-forwarded-proto"] === "string" && /^(?:http|https)$/.test(req.headers["x-forwarded-proto"]) ? req.headers["x-forwarded-proto"] : "http";
  return `${forwardedProto}://${hostHeader}`;
}
function classroomJoinUrls(sessionId, baseUrl = publicBaseUrl) {
  const record = sessionRecords.get(sessionId);
  if (!record) throw new Error("session-not-found");
  const student = new URL("/student/index.html", baseUrl);
  student.searchParams.set("mode", "classroom");
  student.searchParams.set("session", record.sessionLocator);
  student.searchParams.set("code", credentialValueFor("student"));
  const observer = new URL("/observer/index.html", baseUrl);
  observer.searchParams.set("session", record.sessionLocator);
  observer.searchParams.set("code", credentialValueFor("observer"));
  return { student: student.toString(), observer: observer.toString() };
}
function displayJoinUrl(sessionId, baseUrl = publicBaseUrl) {
  const record = sessionRecords.get(sessionId);
  if (!record) throw new Error("session-not-found");
  const url = new URL("/display/index.html", baseUrl);
...
async function publicStage(stage) {
  ...
  const widget = payload.widget && typeof payload.widget === "object" && payload.widget.selector === "selected-artifact"
    ...
    ? {
      ...
      ...(Array.isArray(payload.widget.liveFrames) && payload.widget.liveFrames.length
        ? {
          liveFrames: payload.widget.liveFrames.slice(0, 4).map((item) => ({
            subjectId: typeof item?.subjectId === "string" ? item.subjectId : "anon:unknown",
            seq: Number.isInteger(item?.seq) ? item.seq : 0,
            payload: item?.payload && typeof item.payload === "object"
              ? { objects: item.payload.objects && typeof item.payload.objects === "object" ? Object.fromEntries(Object.entries(item.payload.objects).slice(0, 5)) : {} }
              : { objects: {} },
          })),
        }
        : {}),
  ...
  return {
    ...
    payload: {
      ...
      sources: Array.isArray(payload.sources) ? payload.sources.filter((item) => item === "presentation" || item === "student-live") : null,
...
async function sendClassroomSnapshot(peer) {
  const { context } = peer;
  const baseUrl = peer.publicBaseUrl ?? connections.get(context.connectionId)?.publicBaseUrl ?? publicBaseUrl;
  ...
  if (context.role === "teacher" || isPublicViewer(context)) {
    sendTo(peer, { type: "presentation.sync", audience: context.role === "teacher" ? "teacher-runtime" : context.role, state: publicPresentationState(authoritativePresentationState(context.sessionId)), reason: "reconnect" });
  }
  if (context.role === "teacher") {
    const record = sessionRecords.get(context.sessionId);
    sendTo(peer, { type: "classroom.course", course: record?.course ?? null });
    sendTo(peer, { type: "classroom.display-link", url: displayJoinUrl(context.sessionId, baseUrl) });
    sendTo(peer, {
      type: "classroom.join-links",
      links: { ...classroomJoinUrls(context.sessionId, baseUrl), studentCode: credentials.find((item) => item.role === "student")?.credentialValue ?? null, observerCode: credentials.find((item) => item.role === "observer")?.credentialValue ?? null },
    });
  }
  if (context.role === "display") sendTo(peer, { type: "classroom.join-links", links: classroomJoinUrls(context.sessionId, baseUrl) });
  ...
  try {
    ...
    if (url.pathname === "/healthz") {
      return sendJson(res, 200, {
        ok: true,
        service: "authenticated-classroom-server",
        revision: "R3.10",
        runtime: "authenticated-classroom-server",
        runtimeMode: "authenticated-classroom-server",
        arch: process.arch,
    ...
    if (url.pathname === "/readyz") {
      return sendJson(res, 200, {
        ...
        authentication: true,
        productReady: true,
        currentActivityId: sessionRecords.get(activeSessionId)?.session.currentActivityId ?? session.currentActivityId,
        activeSessionId,
      });
    }
    ...
    if (url.pathname === "/api/classroom/join" && req.method === "POST") {
      const body = await readJson(req);
      const joinRequest = body.requestedRole === "student" ? resolveStudentJoinRequest(body) : body;
      const result = await runtime.join(joinRequest);
    ...
    if (req.method === "POST" && apiParts[0] === "api" && apiParts[1] === "courses" && apiParts[2] && apiParts[3] === "resources") {
      ...
      if (/\.pptx?$/i.test(name) || mimeType.includes("presentation")) {
        const created = presentationLibrary.createPresentation({
          ownerUserId: presentationOwnerId,
          title: result.course.title,
          bytes: body,
          mimeType,
          document: { format: "web-ppt-ooxml-v1", idPrefix: `course-${result.course.slug}`, deckId: `deck:${result.course.slug}` },
        });
    ...
    if (req.method === "POST" && apiParts[0] === "api" && apiParts[1] === "courses" && apiParts[2] && apiParts[3] === "start") {
      ...
      return sendJson(res, 201, await startCourse(courseId, publicBaseUrlForRequest(req)));
    }
  ...
  const connection = acceptWebSocketUpgrade(req, socket, head, async (text, ws) => {
    ...
    if (!context) {
      ...
      try {
        ...
        connections.set(context.connectionId, { ws, context, publicBaseUrl: publicBaseUrlForRequest(req) });
        ...
        if (context.role === "teacher") ws.send({ type: "activity.options", activities: activityControlOptions() });
        ...
        await sendClassroomSnapshot({ ws, context, publicBaseUrl: publicBaseUrlForRequest(req) });
        ...
        ws.send({ type: "server.hello", ok: false, reason: error instanceof Error ? error.message : String(error) });
    ...
    try {
      ...
      if (message.type === "teacher.activity.set") {
        const activityId = typeof message.activityId === "string" ? message.activityId : "";
        if (!activities.some((item) => item.activityId === activityId)) throw new Error("activity-not-found");
        const activity = await runtime.setCurrentActivity(context, activityId, typeof message.expectedRevision === "number" ? message.expectedRevision : undefined);
        const sessionRecord = sessionRecords.get(context.sessionId);
        const authoritativeSession = runtime.authority.getSession(context.sessionId);
        if (sessionRecord && authoritativeSession) sessionRecord.session = authoritativeSession;
        ws.send({ type: "activity.set.ack", ok: true, activity });
        broadcastCurrentActivity(context.sessionId, activity);
        return;
      }
      ...
      if (message.type === "teacher.selection.random") {
        ...
        const stage = await runtime.setStage(
          context,
          "core:teacher-stage",
          { source: "student-live", selectedParticipantIds, widget: selectedLiveWidget(context.sessionId, selectedParticipantIds), annotation: null },
      ...
      if (message.type === "teacher.selection.set") {
        ...
        const stage = await runtime.setStage(
          context,
          "core:teacher-stage",
          { source: "student-live", selectedParticipantIds: selected, widget: selectedLiveWidget(context.sessionId, selected), annotation: null },
          typeof message.expectedRevision === "number" ? message.expectedRevision : undefined,
        );
      ...
      if (message.type === "teacher.stage.composite") {
        const requestedSources = Array.isArray(message.sources) ? [...new Set(message.sources.filter((item) => item === "presentation" || item === "student-live"))] : [];
        const sources = requestedSources.length ? requestedSources : ["student-live"];
        const selectedParticipantIds = Array.isArray(message.selectedParticipantIds) ? [...new Set(message.selectedParticipantIds.filter((item) => typeof item === "string"))].slice(0, 4) : [];
        const playback = sources.includes("presentation") ? authoritativePresentationState(context.sessionId) : null;
        const stage = await runtime.setStage(context, "core:teacher-stage", {
          source: sources.length > 1 ? "composite" : sources[0],
          sources,
          selectedParticipantIds,
          widget: sources.includes("student-live") ? selectedLiveWidget(context.sessionId, selectedParticipantIds) : null,
      ...
      if (message.type === "teacher.stage.source") {
        const source = message.source === "presentation" ? "presentation" : message.source === "activity" ? "activity" : message.source === "entry" ? "entry" : "student-live";
        ...
        const stage = await runtime.setStage(context, source === "presentation" ? "presentation:deck" : "core:teacher-stage", {
          ...
          sources: [source],
          ...(source === "student-live"
            ? { selectedParticipantIds, widget: selectedLiveWidget(context.sessionId, selectedParticipantIds) }
            : source === "activity"
            ? { selectedParticipantIds: [], widget: currentActivityWidget(context.sessionId) }
            : { selectedParticipantIds: [], widget: null }),
          ...(playback ? { presentationRevisionId: playback.presentationRevisionId, presentationSceneId: playback.sceneId, presentationStep: playback.step, presentationPlayState: playback.playState } : {}),
        }, typeof message.expectedRevision === "number" ? message.expectedRevision : undefined);
        ws.send({ type: "stage.state", audience: "teacher-runtime", stage });
        await broadcastPublicStage(context.sessionId, stage);
      ...
      if (message.type === "live.state") {
        ...
        if (participantId) {
          ...
          summaries.set(participantId, { participantId, seq: Number.isInteger(frame.seq) ? frame.seq : 0, objectCount: Object.keys(liveFrameObjects(frame)).length, frame });
        ...
        const currentStage = await runtime.currentStage(context.sessionId);
        const stagePayload = currentStage?.payload && typeof currentStage.payload === "object" ? currentStage.payload : null;
        if (currentStage && stagePayload?.source === "student-live" && Array.isArray(stagePayload.selectedParticipantIds) && stagePayload.selectedParticipantIds.includes(participantId)) {
          const refreshedStage = { ...currentStage, payload: { ...stagePayload, widget: selectedLiveWidget(context.sessionId, stagePayload.selectedParticipantIds) } };
          broadcast(context.sessionId, { type: "stage.state", audience: "teacher-runtime", stage: await teacherStage(refreshedStage) }, (item) => item.role === "teacher");
          await broadcastPublicStage(context.sessionId, refreshedStage);
        }
        return;
      }
    ...
    if (context) {
      ...
      try {
        runtime.authority.disconnect(context.connectionId);
      } catch {}
      broadcastPresence(context.sessionId);
