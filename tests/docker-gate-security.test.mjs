...
test('LAN probe fails closed when local-tools is reachable on a non-loopback host address', async () => {
  const fakeFetch = async (url) => ({ ok: url.includes(':8787') || url.includes(':8788') });
  await assert.rejects(() => probeHostLanPublication({ addresses: ['192.168.1.8'], classroomPort: 8787, localToolsPort: 8788, fetchFn: fakeFetch }), /SECURITY_ASSERTION_FAILED/);
});
test('LAN probe accepts classroom publication when local-tools is unreachable', async () => {
  const fakeFetch = async (url) => { if (url.includes(':8788')) throw new Error('unreachable'); return { ok: true }; };
  const result = await probeHostLanPublication({ addresses: ['192.168.1.8'], classroomPort: 8787, localToolsPort: 8788, fetchFn: fakeFetch });
