...
- Classroom 8787 与 Local Tools 8788 的容器运行路径可用；
- 8788 在宿主只绑定 `127.0.0.1`；
...
2. 使用正式 Compose 或 RC 启动方式启动 Classroom Server，确认 `http://127.0.0.1:8788/healthz` 正常。
3. 在 Mac 上确认 `http://<Mac局域网IPv4>:8787/healthz` 正常；同时确认 `http://<Mac局域网IPv4>:8788/healthz` **不可访问**。
4. 至少使用 1 台学校 XP21A 打开 `http://<Mac局域网IPv4>:8787`，完成页面加载、加入课堂、一次交互、一次提交/事件发送、断网后恢复。
...
不要为了绕过网络问题把 8788 暴露到 LAN，也不要把 Local Tools 合并回 Classroom 端口。
