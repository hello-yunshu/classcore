...
| 领域 | 当前状态 | 说明 |
| --- | --- | --- |
| Storage | `model-tested` | staged claim quota、dedupe、soft-delete purge、GC、bounded cache、hash repair/reconcile |
| Freeze | `model-tested` | current-Draft-only、server fingerprint、web-ppt document/RuntimeIndex/identity-neutral binding 校验 |
| Library | `browser-bundle-built` | 新建、导入、搜索、编辑、重命名、副本、导出、历史、删除入口；reference owner header 仍非正式认证 |
| Classroom runtime | `model-tested` | Session Pin、Prepare、Controller Lease、durable PlaybackState、Display reconnect |
